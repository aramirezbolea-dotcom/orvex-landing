...
import { PLANS, euros, type PlanKey } from "@/lib/plans";
import { currentPrice, type LaunchOffer } from "@/lib/launch-offer";
...
export default function ContactPage({ offer }: { offer: LaunchOffer }) {
  return (
    <section className="bg-gradient-to-b from-gray-50 to-white py-20 sm:py-28">
      <div className="mx-auto max-w-6xl px-4">
        <div className="grid gap-12 lg:grid-cols-2">
          ...
          {/* Form */}
          <Suspense fallback={<ContactForm initialPlan="UNKNOWN" offer={offer} />}>
            <ContactFormFromQuery offer={offer} />
          </Suspense>
        </div>
      </div>
    </section>
...
function ContactFormFromQuery({ offer }: { offer: LaunchOffer }) {
  const plan = useSearchParams().get("plan");
  return <ContactForm initialPlan={isPlanValue(plan) ? plan : "UNKNOWN"} offer={offer} />;
}

function ContactForm({ initialPlan, offer }: { initialPlan: PlanValue; offer: LaunchOffer }) {
            ...
            >
              ...
              {(Object.keys(PLANS) as PlanKey[]).map((key) => (
                <option key={key} value={key}>
                  {offer.active
                    ? `${PLANS[key].name} — ${euros(currentPrice(PLANS[key].price, offer))} (antes ${euros(PLANS[key].price)})`
                    : `${PLANS[key].name} — ${euros(PLANS[key].price)}`}
                </option>
              ))}
            </select>
          </div>
